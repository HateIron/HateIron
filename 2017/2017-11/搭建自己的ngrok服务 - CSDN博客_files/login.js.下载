    ...
    "<div class='csdn_loginbox' style='z-index: 100'>"
    	+ "<div class='login_content' style='float:left;'>"
    	  + "<div class='login_content_inner'>"
    		+ "<div style='font-size: 12px;'>CSDN帐号登录 </div>"
    		+ "<input name='close' type='button' class='close'  onclick='javascript:csdn.closeLogin();return false;'  />"
    		+ "<p class='point' id='sperr'></p>"
    		+ "<input name='user_id'  id='u'  type='text' class='user_id' placeholder='输入用户名/邮箱/手机号' />"
    		+ "<input name='password' id='p' type='password' class='password' placeholder='密码'/ >"
    		+ "<label class='next_login'><p class='remember clearfix'><input name='checkbox'  id='chkre' type='checkbox' value='checkbox' /><em>下次自动登录</em><a href='https://" + csdn.domain + "/account/forgotpassword' target='_blank' style='float:right;' >忘记密码？</a></p></label>"
    		//+ "<div class='pw_lg'>"
    			//+ "<a href='https://" + csdn.domain + "/account/forgotpassword' target='_blank' >忘记密码？</a>"
    		//+ "</div>"
            ...
            +"<a href='https://" + csdn.domain + "/account/mobileregister' target='_blank' class='pw_a'>立即注册</a>"
...
csdn.send_code = function(){
    ...
    csdn.post('https://' + csdn.domain + "/account/fpwd?action=sendLoginMobileCode&mobile=" + csdn.$("mobile").value + "&callback=csdn.send_code_callback");
}
